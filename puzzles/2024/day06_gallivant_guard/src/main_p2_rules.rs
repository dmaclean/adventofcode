...
fn check_direction_for_loop(
    ...
    loop {
        ...
        // Check if we've found a previous visit in the same direction
        if let Some(visit) = visits.iter().find(|v| v.x == x_usize && v.y == y_usize) {
            return visit.direction == direction;
        }
    }
}

fn main() {
    let path = Path::new("sample_input.txt");
